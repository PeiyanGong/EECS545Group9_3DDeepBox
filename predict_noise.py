...
def parse_args():
    ...
    parser.add_argument('--noise',dest = 'noise',default= '10')
...
def test(model, image_dir, box2d_loc, box3d_loc,noise):
    dim_stats = read_stats("label_stats.txt")
    ### buile graph
    # dimension, orientation, confidence, loss, optimizer, loss_d, loss_o, loss_c = build_model()
    ...
    for f in all_image:
        ...
        with open(box3d_file, 'w') as box3d:
            ...
            for line in open(box2d_file):
                ...
                # Add random noise on 2d bbox 

                xmin = int(float(line[4])) + np.random.randint(noise)-(noise/2)
                ymin = int(float(line[5])) + np.random.randint(noise)-(noise/2)
                xmax = int(float(line[6])) + np.random.randint(noise)-(noise/2)
                ymax = int(float(line[7])) + np.random.randint(noise)-(noise/2)
...
if __name__ == "__main__":
    ...
    if args.mode == 'train':
        ...
        test(args.model, args.image, args.box2d, args.output,int(args.noise))
