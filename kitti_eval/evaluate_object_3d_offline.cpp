...
inline double groundBoxOverlap(tDetection d, tGroundtruth g, int32_t criterion = -1) {
    ...
    union_(gp, dp, un); // HERE!
    // cout<< __LINE__<<endl;
    double inter_area = in.empty() ? 0 : area(in.front());
    // cout<< __LINE__<<endl;
    // cout<<"Is empty: "<<un.empty()<<endl; // un is EMPTY!
    // cout<<"Check: "<<area(un.front())<<endl;
    double union_area = area(un.front()); // BREAK AT HERE
    // cout<< __LINE__<<endl;
    double o;
    if(criterion==-1)     // union
  ...
  for(int32_t i=0; i<gt.size(); i++){
    ...
    for(int32_t j=0; j<det.size(); j++){
      ...
      if(ignored_threshold[j])
        continue;
      // cout<< __LINE__ <<endl;
      // cout<< "Here: "<<det[j].h << " " << gt[i].h <<endl;
      // find the maximum score for the candidates and get idx of respective detection
      // cout<< __LINE__ <<endl;
      double overlap = boxoverlap(det[j], gt[i], -1); // BREAK AT HERE
      // cout<< __LINE__ <<endl;
...
bool eval_class (FILE *fp_det, FILE *fp_ori, CLASSES current_class,
  ...
  for (int32_t i=0; i<groundtruth.size(); i++){
    ...
    // cout<< __LINE__ <<endl;
    pr_tmp = computeStatistics(current_class, groundtruth[i], detections[i], dc, i_gt, i_det, false, boxoverlap, metric); // BREAK AT HERE
    // cout<< __LINE__ <<endl;
